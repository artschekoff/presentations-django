...
- Start a Celery worker with `celery -A presentations worker --loglevel=info --pool=threads --concurrency=4` (remember to set `CELERY_BROKER_URL`/`CELERY_RESULT_BACKEND` or rely on the default Redis URL).
